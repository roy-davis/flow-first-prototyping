...
# Flow: Choose cloud provider

> File: docs/ux-ui/flows/clara-chooses-cloud-provider.md
> Primary actor: Clara the Cloud Collector
> Story: clara-chooses-cloud-provider.md
> Journey: clara-connects-cloud-library.md
> Implements: Choosing provider

- Trigger: Clara starts Add source for cloud audiobooks.
- Entry point: Screen/state implied by Choosing provider.
- Preconditions: Relevant source, cache, or selection state exists; user has reached the source screen for this story.
- Steps:
  1. Actor enters the relevant screen for Choosing provider.
  2. The app renders the current state and required data for this single task slice.
  3. Actor takes the primary action for "Choose cloud provider".
...
- Exit states: Provider Authorization opens for the selected provider.
- Completion criteria: Provider Authorization opens for the selected provider.
