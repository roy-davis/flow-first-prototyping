...
# Flow: Review discovered book

> File: docs/ux-ui/flows/leo-reviews-discovered-book.md
> Primary actor: Leo the Living Room Listener
> Story: leo-reviews-discovered-book.md
> Journey: leo-discovers-and-adds-book.md
> Implements: Inspecting a discovered book

- Trigger: Leo opens a discovered book from Discovered Books.
- Entry point: Screen/state implied by Inspecting a discovered book.
- Preconditions: Relevant source, cache, or selection state exists; user has reached the source screen for this story.
- Steps:
  1. Actor enters the relevant screen for Inspecting a discovered book.
  2. The app renders the current state and required data for this single task slice.
  3. Actor takes the primary action for "Review discovered book".
...
- Exit states: Leo is ready to add the book, skip it, or leave it discovered.
- Completion criteria: Leo is ready to add the book, skip it, or leave it discovered.
