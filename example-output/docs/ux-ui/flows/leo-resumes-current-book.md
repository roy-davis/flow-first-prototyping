...
# Flow: Resume current book

> File: docs/ux-ui/flows/leo-resumes-current-book.md
> Primary actor: Leo the Living Room Listener
> Story: leo-resumes-current-book.md
> Journey: leo-resumes-current-book.md
> Implements: Checking resume context

- Trigger: Leo opens the app after a prior session.
- Entry point: Screen/state implied by Checking resume context.
- Preconditions: Relevant source, cache, or selection state exists; user has reached the source screen for this story.
- Steps:
  1. Actor enters the relevant screen for Checking resume context.
  2. The app renders the current state and required data for this single task slice.
  3. Actor takes the primary action for "Resume current book".
...
- Exit states: Playback begins at the saved position or Leo sees a clear repair entry.
- Completion criteria: Playback begins at the saved position or Leo sees a clear repair entry.
