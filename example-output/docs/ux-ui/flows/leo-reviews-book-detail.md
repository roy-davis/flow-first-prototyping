...
# Flow: Review book detail

> File: docs/ux-ui/flows/leo-reviews-book-detail.md
> Primary actor: Leo the Living Room Listener
> Story: leo-reviews-book-detail.md
> Journey: leo-browses-and-starts-book.md
> Implements: Confirming a book

- Trigger: Leo selects a focused book from Library.
- Entry point: Screen/state implied by Confirming a book.
- Preconditions: Relevant source, cache, or selection state exists; user has reached the source screen for this story.
- Steps:
  1. Actor enters the relevant screen for Confirming a book.
  2. The app renders the current state and required data for this single task slice.
  3. Actor takes the primary action for "Review book detail".
...
- Exit states: Leo sees enough context to start, resume, choose a chapter, or repair source.
- Completion criteria: Leo sees enough context to start, resume, choose a chapter, or repair source.
