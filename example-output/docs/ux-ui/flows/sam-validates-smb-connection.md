...
# Flow: Validate SMB connection

> File: docs/ux-ui/flows/sam-validates-smb-connection.md
> Primary actor: Sam the Server Steward
> Story: sam-validates-smb-connection.md
> Journey: sam-connects-smb-library.md
> Implements: Validating SMB connection

- Trigger: SMB Setup opens from Sources or Source Repair.
- Entry point: Screen/state implied by Validating SMB connection.
- Preconditions: Relevant source, cache, or selection state exists; user has reached the source screen for this story.
- Steps:
  1. Actor enters the relevant screen for Validating SMB connection.
  2. The app renders the current state and required data for this single task slice.
  3. Actor takes the primary action for "Validate SMB connection".
...
- Exit states: The connection validates or Sam sees host/share/credential-specific recovery.
- Completion criteria: The connection validates or Sam sees host/share/credential-specific recovery.
